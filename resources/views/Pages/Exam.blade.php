...
<body class="w-full h-[100dvh] overflow-hidden bg-[#26AF5A] flex flex-col">
    <header class="w-full h-[80px] bg-[#1E7B42] flex items-center px-6">
        ...
        <div class="w-[70%] flex flex-col items-center justify-center text-white">
            <h4 class="font-bold text-2xl tracking-wide">{{ $category }}</h4>
            <span class="text-xs"> Questions 1 / 50 </span>
        </div>

        <div class="w-[15%] h-full flex items-center justify-end">
            <svg id="textToSpeech_Trigger" class=" w-[30px] h-[30px] text-white  cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <script>
        ...
        const textToSpeech_Trigger = document.getElementById('textToSpeech_Trigger');

        textToSpeech_Trigger.addEventListener('click', function () {
            const q = quiz[current];
            

            const speech = new SpeechSynthesisUtterance();
            speech.text = `${q.question}. A: ${q.choices[0]}. B: ${q.choices[1]}. C: ${q.choices[2]}. D: ${q.choices[3]}`;
            speech.lang = "en-US";
            speech.rate = 1.1;
            speech.pitch = 1;
